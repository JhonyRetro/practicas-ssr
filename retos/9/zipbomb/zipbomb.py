...
def copy_and_compress(infile, outfile, n_copies):
	zf = zipfile.ZipFile(outfile, mode='w', allowZip64= True)
	for i in range(n_copies):
		f_name = '%s-%d.%s' % (get_filename(infile),i,get_file_extension(infile))
...
if __name__ == '__main__':
	...
	for i in range(1,n_levels+1):
		copy_and_compress('%d.zip'%i,'%d.zip'%(i+1),10)
	...
	decompressed_size = decompressed_size / 10**9
	print('Size After Decompression: %d EB'%decompressed_size)
